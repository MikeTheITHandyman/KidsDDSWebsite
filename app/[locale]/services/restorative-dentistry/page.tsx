...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'restorativeDentistryPage' })
  return {
    title: t('metaTitle'),
    description: t('metaDescription'),
    alternates: { canonical: localizedUrl('/services/restorative-dentistry', locale), languages: localeAlternates('/services/restorative-dentistry') },
    openGraph: {
      title: t('metaTitle'),
      description: t('metaDescription'),
      url: localizedUrl('/services/restorative-dentistry', locale),
          ...
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
              <Link
                ...
                style={{ display: 'inline-flex', alignItems: 'center', gap: '0.45rem', background: 'linear-gradient(135deg, var(--accent-500) 0%, var(--cta-coral) 100%)', color: '#fff', fontFamily: 'Nunito, sans-serif', fontWeight: 800, fontSize: '0.95rem', padding: '0.85rem 2rem', borderRadius: '100px', textDecoration: 'none', boxShadow: '0 6px 22px rgba(239,108,26,0.52)' }}
              >
                {t('bookAppointment')}
              </Link>
              <Link
